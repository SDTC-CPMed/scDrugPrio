...
#source("~/scDrugPrio/SIGMA_201213/Doctis_anti_IL17_v1/R/Network_predictions.R")
...
# INPUT
same_drugs <- as.matrix(read.table(file ="../Input/Drugs/SAME_DRUGS_PsA.txt", sep="\t", header = T, stringsAsFactors = F))
drugs <- read.table(file = "../Input/Drugs/DRUG_TARGET_MATRIX_PsA.txt", sep="\t", header = T, stringsAsFactors = F)
psa_drugs <- unique(read.table(file = "../Input/Drugs/PsA_drugs_from_DrugBank.txt", sep="\t", header = T, stringsAsFactors = F)[,1])
...
out_dir <- "../Output/Network_distances/Responder/"
...
separate_unique_drug_target_combinations_into_individual_drugs(files = lf, 
                                                               ...
                                                               disease_specific_drugs = psa_drugs, 
...
lf <- lf[grepl(lf, pattern = "_R_")]
# get paths to average closest netork distances
drug_dists <- list.files("../Output/Network_distances/Responder/", pattern = "INDIVIDUAL_DRUGS_", full.names = T)
# remove deg files that are empty
temp <- colnames(read.table(file = "../Output/DCA_MAST_DEGs/SUMMARY_MAST_DEGs_all_clusters_R_TRANSLATED.txt", sep = "\t", header = T))
...
save_name <- unlist(strsplit(save_name, split = "../Output/DCA_MAST_DEGs//"))
save_name <- save_name[grepl(save_name, pattern = "_R_")]
save_name <- paste("FC_INDIVIDUAL_DRUGS_", save_name, sep="")
# make output folder
out_dir <- "../Output/FC_criteria_checking/Responder/"
...
# seperate unique drug target combinations into individual drugs again
out_dir <- "../Output/Network_distances/Non_responder/"
lf  <- list.files(path = out_dir, pattern = "drug-disease_closest_distances_vs_random_bin_adjusted__")
separate_unique_drug_target_combinations_into_individual_drugs(files = lf, 
                                                               remove_part_of_file_name_for_output = "drug-disease_closest_distances_vs_random_bin_adjusted__",
                                                               same_drugs = same_drugs, 
...
# translate DEG files
lf <- list.files(path = "../Output/DCA_MAST_DEGs/", pattern = "Cluster_", full.names = T)
lf <- lf[grepl(lf, pattern = "_NR_")]
# get paths to average closest netork distances
drug_dists <- list.files("../Output/Network_distances/Non_responder/", pattern = "INDIVIDUAL_DRUGS_", full.names = T)

# remove deg files that are empty
temp <- colnames(read.table(file = "../Output/DCA_MAST_DEGs/SUMMARY_MAST_DEGs_all_clusters_NR_TRANSLATED.txt", sep = "\t", header = T))
temp <- paste(temp, "_", sep = "")
temp2 <- rep(F, times = length(lf))
for(i in 1:length(temp)){
  temp2[grepl(lf, pattern = temp[i])] <- T
...
save_name <- unlist(strsplit(lf, split = ".txt"))
save_name <- unlist(strsplit(save_name, split = "../Output/DCA_MAST_DEGs//"))
save_name <- save_name[grepl(save_name, pattern = "_NR_")]
save_name <- paste("FC_INDIVIDUAL_DRUGS_", save_name, sep="")
# make output folder
out_dir <- "../Output/FC_criteria_checking/Non-responder/"
dir.create(out_dir, showWarnings = F)
