...
# INPUT
lit_data <- as.matrix(read.table(file = "../Input/Literature_search_drugs/Filtered_drug_effects_221013.txt",sep="\t", header = T, stringsAsFactors = F))

# GET FC SUMMARY FOR MS DATA
temp <- as.matrix(read.table(file = "../Output/FC_criteria_checking/Responder/SUMMARY_only_drugs_passing_network_criteria.txt",sep= "\t", stringsAsFactors = F, header = T))
