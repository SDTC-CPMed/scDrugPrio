#' Summarize pharmacological effects on targets
#'
#' Matching pharmacological action and fold change of targeted DEGs to potential candidates drug targets
#'
#' @param input matrix with 8 columns listing individual drug's network distances. Column 1 = DrugBank ID, column 2 = known drug for this disease (either "FALSE" OR "TRUE"), column 3 = n of drug targets, column 4 = dc, column 5 = zc, column 6 = mean random dc, column 7 = SD of random dc, column 8 = P value for significance of dc. Only column 1 is used for merging information in this function, though the matrix is reshuffled before return and therefore requires all 8 columns.
#' @param degs matrix with 6 columns listing DEGs associated with the input file. Preserves output format of Seurat based DEG calculation. Column 1 = gene name in annotation that matches 'drug_effect', column 3 = average log fold change (avg_logFC), column 6 = adjusted P value.
#' @param drug_effect matrix with 3 columns listing drug target information derived from DrugBank. Column 1 = DrugBankID used for matching with 'input', column 2 = gene name in annotation that matches with 'degs', column 3 = pharmacological action on the drug target.
#'
