...
cd_drug <- unique(read.table(file = "../Input/Drugs/CD_drugs_from_DrugBank_201015.txt", sep="\t", header = T, stringsAsFactors = F, quote = "")[,1])
cd_drug <- cd_drug[cd_drug != ""]
...
out_dir <- "../Output/CD/DCA_MAST_DEGs_predictions/literature_PPI"
...
separate_unique_drug_target_combinations_into_individual_drugs(files = lf, 
                                                               ...
                                                               disease_specific_drugs = cd_drug, 
...
# translate DEG files
lf <- list.files(path = "../Input/CD/DCA_MAST_DEGs_logFC_1.5_CD_ileal_cells/", pattern = "Cluster_", full.names =F)
...
for(i in 1:length(lf)){
  temp <- read.table(file = paste("../Input/CD/DCA_MAST_DEGs_logFC_1.5_CD_ileal_cells/",lf[i], sep=""), sep = "\t", header = T, stringsAsFactors = F)
...
drug_dists <- list.files("../Output/CD/DCA_MAST_DEGs_predictions/literature_PPI/", pattern = "INDIVIDUAL_DRUGS_", full.names = T)
# remove deg files that are empty
temp <- colnames(read.table(file = "../Input/CD/DCA_MAST_DEGs_logFC_1.5_CD_ileal_cells/TRANSLATED_HGNC_SYMBOL_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep = "\t", header = T))
...
# REUSE PREVIOUS LITERATURE EVIDENCE FOR PHARMACOLOGICAL ACTION
##################################################################################
