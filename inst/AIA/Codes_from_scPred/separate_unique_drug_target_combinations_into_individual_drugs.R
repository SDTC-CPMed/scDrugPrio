#' Separation of unique drug targets to individual drugs.
#'
...
#' separate_unique_drug_target_combinations_into_individual_drugs(files = "data/test.txt", same_drugs = same_drugs, disease_specific_drugs = disease_specific_drugs)
#'
separate_unique_drug_target_combinations_into_individual_drugs <- function(files,
                                                                           same_drugs,
                                                                           disease_specific_drugs,
                                                                           in_dir = getwd(),
                                                                           out_dir = getwd(),
                                                                           output_file_name_add_on = "",
                                                                           remove_part_of_file_name_for_output = "drug-disease_closest_distances_vs_random_bin_adjusted\\__",
                                                                           cores = 1){

  if(in_dir != ""){
    if(substr(x = in_dir, start = nchar(in_dir), stop = nchar(in_dir)) != "/"){
  ...
  foreach(file = c(1:length(files)), .export = NULL) %dopar% { # for every file
    ...
    if(any(is.na(in_file[,7]))){
      if(sum(is.na(in_file[,7])) == 1){
        temp <- matrix(in_file[is.na(in_file[,7]),], nrow = 1)
        colnames(temp) <- colnames(in_file)
      } else {
  ...
  print("Drug target combinations successfully separated into individual drugs")
}


# USED FOR TESTING
# separate_unique_drug_target_combinations_into_individual_drugs(
