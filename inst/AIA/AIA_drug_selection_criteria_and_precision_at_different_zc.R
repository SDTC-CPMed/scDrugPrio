...
out <- foreach(i = (1:length(lf)), .combine = "rbind") %do% {
  ...
  temp <- unlist(strsplit(strsplit(lf[i], split = "_")[[1]][4], split = ".txt.txt"))[1] # get cluster number
...
clust_colors <- read.table(file = "../Output/Clustering/Cluster_colors.txt",sep="\t", stringsAsFactors = F, header = T)
...
prec_rec_matrix <- foreach(c = c(1:length(cl)), .combine = "rbind") %do% {
  ...
  prec_rec <- foreach(cut = c(1:length(cut_offs)), .combine = "rbind") %do% {
    ...
    recall <- sum(grepl(temp2, pattern = "TRUE"))/n_RA
...
ggsave(filename = paste(dir, "PRECISION_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 150, height = 80, units = "mm")
...
p <- ggplot(prec_rec_matrix, aes(x = cut_off, y = recall, color = Cluster)) + 
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.key.size = unit(0.7,"lines"),
        legend.title = element_text(face = "bold", size = 10, color = "black"))
ggsave(filename = paste(dir, "RECALL_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 150, height = 80, units = "mm")
...
p <- ggplot(prec_rec_matrix, aes(x = recall, y = precision, color = Cluster)) + 
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.key.size = unit(0.7,"lines"),
        legend.title = element_text(face = "bold", size = 10, color = "black"))
ggsave(filename = paste(dir, "RECALL_PRECISION_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 150, height = 80, units = "mm")
...
z_0.15_criterium <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  ...
  rec <- sum(grepl(temp, pattern = "TRUE")) / n_RA
...
z_d_fc_criteria <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  if(any(fc_criterium_file[,19]==paste("FC_INDIVIDUAL_DRUGS_Cluster_",cl[i],"_res=0.6_dims=32_k=15",sep=""))){
    temp <- fc_criterium_file[fc_criterium_file[,19]==paste("FC_INDIVIDUAL_DRUGS_Cluster_",cl[i],"_res=0.6_dims=32_k=15",sep=""),]
    temp <- temp[as.numeric(temp[,12]) > 0,3] # choose only drugs that counteract DEGs
    if(length(temp)>0){
      prec <- sum(temp == "TRUE") / length(temp)
      rec <- sum(temp == "TRUE") / n_RA
    } else {
      prec <- rec <- 0
    }
  } else {
