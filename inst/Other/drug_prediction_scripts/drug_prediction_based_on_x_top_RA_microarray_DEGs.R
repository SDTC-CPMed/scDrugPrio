# Drug prediction based on x top bulk RNA-seq DEGs
...
mode(degs) <- "character"
DEG <- degs
rm(bulk_synovium, bulk_blood, degs)
...
##################################################################################################################
print("START literature PPI network analysis of top microarray DEGs")
out.dir <- paste(fp, "/../Output/RA_benchmark_predictions/literature_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "Microarray_DEGs_in_literature_PPI"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){ # for every microarray data set
  for(j in 1:length(x)){ # using x top genes
    if(sum(!is.na(degs[,i]))>=x[j]){
      ...
      if(!(length(out)==1) & !all(is.na(out))){
        ...
        print(out)
        write.table(out, file = paste(out.dir,"/precision_recall_matrix_", save_name, "_", colnames(degs)[i], "_top_", x[j],"_microarray_DEGs.txt", sep=""), sep="\t", col.names = T, row.names = F)
      } else {
...
##################################################################################################################
print("START HuRI PPI network analysis of top microarray DEGs")
out.dir <- paste(fp, "/../Output/RA_benchmark_predictions/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
source(paste(fp, "/in_silico_drug_efficacy_screening_algorithm.R",sep="")) # to remove "/temp" folder if exists
setwd(paste(fp, "/..", sep=""))
...
same_drugs <- same_drugs[same_drugs[,1]%in%predefined_list | same_drugs[,2]%in%predefined_list,]
predefined_list <- unique(c(same_drugs[,1], same_drugs[,2], predefined_list)) 
predefined_list <- predefined_list[predefined_list %in% colnames(drugs)]
rm(same_drugs)
# only include DEGs that are found in PPI
degs <- DEG
