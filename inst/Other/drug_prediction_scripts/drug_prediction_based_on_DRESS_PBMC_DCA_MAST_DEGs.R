...
# Load DCA + MAST DEGs
degs <- read.table("Input/DRESS GSE132802/PBMCs/DCA_MAST_DEGs_logFC_1.5_DRESS_PBMCs/TRANSLATED_ENTREZ_ID_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep="\t", header = T)
degs <- as.matrix(degs)
mode(degs) <- "character"
# Set up output directories
dir.create("Output/PBMCs")
dir.create("Output/PBMCs/DCA_MAST_DEGs_predictions")
dir.create("Output/PBMCs/DCA_MAST_DEGs_predictions/literature_PPI")
dir.create("Output/PBMCs/DCA_MAST_DEGs_predictions/HuRI_PPI")

# start drug prediction literature PPI
##################################################################################################################
print("START literature PPI network analysis for PBMCs DEGs")
out.dir <- paste(getwd(), "/Output/PBMCs/DCA_MAST_DEGs_predictions/literature_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "PBMCs_DCA_MAST_DEGs_literature_PPI"
print(save_name)
rec <- F
...
##################################################################################################################
print("START HuRI PPI network analysis of PBMCs DEGs")
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/PBMCs/DCA_MAST_DEGs_predictions/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "PBMCs_DCA_MAST_DEGs_HuRI_PPI"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: DEGs from ", colnames(degs)[i], sep=""))
