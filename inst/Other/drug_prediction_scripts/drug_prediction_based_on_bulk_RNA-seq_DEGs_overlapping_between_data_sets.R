...
# Analysis
save_name <- "bulk_RNA-seq_DEGs_overlapping_between_data_sets_in_literature_PPI_1000"
print(save_name)
rec <- F
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "bulk_RNA-seq_DEGs_overlapping_between_data_sets_in_HuRI_PPI_1000"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: ", colnames(degs)[i], sep=""))
