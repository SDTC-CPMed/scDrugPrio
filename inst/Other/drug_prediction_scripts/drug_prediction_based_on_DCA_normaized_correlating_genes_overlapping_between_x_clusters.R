# Drug prediction based on correlating genes that overlap between x clusters
...
# Analysis
save_name <- "DCA_normalized_correlating_genes_overlapping_between_clusters_in_literature_PPI_1000"
print(save_name)
rec <- F
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "DCA_normalized_correlating_genes_overlapping_between_clusters_in_HuRI_PPI_1000"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
