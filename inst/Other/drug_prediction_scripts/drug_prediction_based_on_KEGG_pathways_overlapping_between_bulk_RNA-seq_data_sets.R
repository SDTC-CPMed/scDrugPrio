# Drug prediction based on KEGG pathways overlapping between bulk RNA-seq data sets (blood and synovium)
...
# CALCULATE: Enriched pathways for all bulk data sets
...
pathways <- kegg_pathways[,pathways]
pathways <- as.vector(pathways)
pathways <- unique(pathways[!is.na(pathways)])

pathways <- matrix(pathways, ncol = 1)
colnames(pathways) <- "Pathways_enriched_in_both_bul_RNA-seq_data_sets"
degs <- pathways
...
# Analysis
save_name <- "bulk_RNA-seq_enriched_pathways_overlapping_between_data_sets_in_literature_PPI_1000"
print(save_name)
rec <- F
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "bulk_RNA-seq_enriched_pathways_overlapping_between_data_sets_in_HuRI_PPI_1000"
print(save_name)
rec <- F
