# Drug prediction based on pathways (sig enriched with DCA normalized correlating genes) that overlap between x clusters
...
write.table(cbind(names(pathways), pathways), file = "Output/DCA_MAST_DEGs_predictions/KEGG_pathways_enriched_by_multiple_cell_types_DCA_normalized_correlation_genes.txt", sep="\t",
            col.names = c("Pathway", "enriched_in_x_cell_types"), row.names = F)
pathways <- pathways[pathways > 1]
print(paste("Overlapping pathways n = ", length(pathways), sep=""))
...
if(is.vector(pathways)){
  pathways <- matrix(pathways, ncol = 1)
}
...
dir.create("Output/Correlation_genes_prediction/literature_PPI")
dir.create("Output/Correlation_genes_prediction/HuRI_PPI")

# start drug prediction literature PPI
##################################################################################################################
print("START literature PPI network analysis")
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/Correlation_genes_prediction/literature_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "DCA_normalized_correlating_genes_enriched_pathways_overlapping_between_clusters_in_literature_PPI_1000"
print(save_name)
rec <- F
...
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/Correlation_genes_prediction/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
source(paste(fp, "/in_silico_drug_efficacy_screening_algorithm.R",sep="")) # to remove "/temp" folder if exists
# LOAD PPI
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "DCA_normalized_correlating_genes_enriched_pathways_overlapping_between_clusters_in_HuRI_PPI_1000"
print(save_name)
rec <- F
