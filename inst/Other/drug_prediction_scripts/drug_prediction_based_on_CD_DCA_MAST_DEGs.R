...
# Load DCA + MAST DEGs
degs <- read.table("Input/CD GSE134809/DCA_MAST_DEGs_logFC_1.5_CD_ileal_cells/TRANSLATED_ENTREZ_ID_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep="\t", header = T)
degs <- as.matrix(degs)
mode(degs) <- "character"
...
##################################################################################################################
print("START HuRI PPI network analysis of CD DEGs")
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/CD/DCA_MAST_DEGs_predictions/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
