#
#
# Drug prediction based on top x MS DEGs calculated based on DCA denoised data with MAST
#
##################################################################################################################
fp <- getwd()
setwd(paste(fp, "/..", sep=""))
...
# Load DCA + MAST DEGs
DEG <- read.table("Input/MS GSE138266/DCA_MAST_DEGs_logFC_1.5_MS/TRANSLATED_ENTREZ_ID_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep="\t", header = T)
DEG <- as.matrix(DEG)
mode(DEG) <- "character"
