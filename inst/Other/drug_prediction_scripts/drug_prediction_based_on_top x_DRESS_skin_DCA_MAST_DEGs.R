#
#
# Drug prediction based on top x DRESS skin DEGs calculated based on DCA denoised data with MAST
#
##################################################################################################################
fp <- getwd()
setwd(paste(fp, "/..", sep=""))
...
# Load DCA + MAST DEGs
DEG <- read.table("Input/DRESS GSE132802/Skin cells/DCA_MAST_DEGs_logFC_1.5_DRESS_SKIN/TRANSLATED_ENTREZ_ID_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep="\t", header = T)
DEG <- as.matrix(DEG)
mode(DEG) <- "character"
print(colSums(!is.na(DEG)))
  ...
  for(j in 1:length(x)){ # using x top genes
    if(sum(!is.na(degs[,i]))>=x[j]){
      ...
      out <- in_silico_drug_efficacy_screening(ppi = ppi, drug_matrix = drugs, disease_module = as.character(degs[1:x[j],i]), disease_module_lcc = F, out.dir = out.dir,
                                               disease_module_name = paste(save_name, "_", colnames(degs)[i], "_top_", x[j],sep=""), cores = 10, recycle = rec, n_random_iterations = 1000)
      #rec <- T
...
for(i in 1:ncol(degs)){ # for every cluster
  for(j in 1:length(x)){ # using x top genes
    if(sum(!is.na(degs[,i]))>=x[j]){
      print(paste("CALCULATING: DEGs from ", colnames(degs)[i], sep=""))
      out <- in_silico_drug_efficacy_screening(ppi = ppi, drug_matrix = drugs, disease_module = as.character(degs[1:x[j],i]), disease_module_lcc = F, out.dir = out.dir,
                                               disease_module_name = paste(save_name, "_", colnames(degs)[i], "_top_", x[j],sep=""), cores = 20, recycle = rec, n_random_iterations = 1000)
      #rec <- T
      if(!(length(out)==1) & !all(is.na(out))){
        out <- precision_and_recall_from_z_score(predefined_list, out)
        print(out)
