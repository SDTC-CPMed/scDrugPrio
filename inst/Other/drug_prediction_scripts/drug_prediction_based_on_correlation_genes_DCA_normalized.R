# Drug prediction based on DCA normalized correlation genes
...
# Analysis
save_name <- "Correlation_genes_in_literature_PPI_1000"
print(save_name)
rec <- F
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "Correlation_genes_in_HuRI_PPI_1000"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: DEGs from ", colnames(degs)[i], sep=""))
