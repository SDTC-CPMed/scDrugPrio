#
#
# Drug prediction based on DRESS PBMC DEGs calculated based on DCA denoised data with MAST
#
##################################################################################################################
fp <- getwd()
setwd(paste(fp, "/..", sep=""))
...
# Load DCA + MAST DEGs
degs <- read.table("Input/MS GSE138266/DCA_MAST_DEGs_logFC_1.5_MS/TRANSLATED_ENTREZ_ID_sig_MAST_DEGs_log(1.5)_all_clusters.txt", sep="\t", header = T)
degs <- as.matrix(degs)
mode(degs) <- "character"
# Set up output directories
dir.create("Output/MS")
dir.create("Output/MS/DCA_MAST_DEGs_predictions")
dir.create("Output/MS/DCA_MAST_DEGs_predictions/literature_PPI")
dir.create("Output/MS/DCA_MAST_DEGs_predictions/HuRI_PPI")

# start drug prediction literature PPI
##################################################################################################################
print("START literature PPI network analysis for MS DEGs")
out.dir <- paste(getwd(), "/Output/MS/DCA_MAST_DEGs_predictions/literature_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
predefined_list <- as.vector(read.table("Input/Drugs/MS_drugs_from_DrugBank.txt", sep="\t", header = T)[,1])
...
# Analysis
save_name <- "MS_DCA_MAST_DEGs_literature_PPI"
print(save_name)
rec <- F
...
##################################################################################################################
print("START HuRI PPI network analysis of MS DEGs")
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/MS/DCA_MAST_DEGs_predictions/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "MS_DCA_MAST_DEGs_HuRI_PPI"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: DEGs from ", colnames(degs)[i], sep=""))
