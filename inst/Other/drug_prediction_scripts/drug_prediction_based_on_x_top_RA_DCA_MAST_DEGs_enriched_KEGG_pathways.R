...
# number of enriched pathways included in the analysis
...
# Load background genes (all genes measured with scRNAseq)
print("LOAD background genes")
bg <- read.table("Input/Human-mouse_homologs/mouse_genes_from_scRNAseq.txt", sep="\t", header = F)
bg <- as.vector(bg[,1])

# Load KEGG pathways
print("LOAD KEGG genes")
...
##################################################################################################################
print("START literature PPI network analysis of enriched KEGG pathways")
...
# Analysis
save_name <- "RA_DCA_MAST_DEGs_enriched_KEGG_pathways_in_literature_PPI"
rec <- F
...
# Analysis
save_name <- "RA_DCA_MAST_DEGs_enriched_KEGG_pathways_in_HuRI_PPI"
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: ", colnames(degs)[i], sep=""))
  # pathway enrichment
