# Drug prediction based on LCCs formed by DCA normalized correlating genes
...
# Analysis
save_name <- "LCCs_from_DCA_normalized_correlating_genes_in_literature_PPI_1000"
print(save_name)
rec <- F
...
ppi[,2] <- as.character(ppi[,2])
# Analysis
save_name <- "LCCs_from_DCA_normalized_correlating_genes_in_HuRI_PPI_1000"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){
  print(paste("CALCULATING: LCCs based on DEGs from ", colnames(degs)[i], sep=""))
