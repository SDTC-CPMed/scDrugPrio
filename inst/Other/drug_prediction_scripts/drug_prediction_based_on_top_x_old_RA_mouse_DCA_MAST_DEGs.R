...
# Load DCA + MAST DEGs
DEG <- read.table("Output/DCA_MAST_DEGs/TRANSLATED_Summary_sig_adj_MAST_DEGs_all_clusters_res=0.6_dims=32.txt", sep="\t", header = T)
DEG <- as.matrix(DEG)
mode(DEG) <- "character"
...
##################################################################################################################
print("START literature PPI network analysis for RA DEGs")
...
# Analysis
save_name <- "old_RA_mouse_DCA_MAST_DEGs_literature_PPI"
print(save_name)
rec <- F
...
setwd(paste(fp, "/..", sep=""))
out.dir <- paste(getwd(), "/Output/old_RA_mouse/DCA_MAST_DEGs_predictions/HuRI_PPI",sep="") # output folder for in_silico_drug_efficacy_screening()
...
# Analysis
save_name <- "old_RA_mouse_DCA_MAST_DEGs_HuRI_PPI"
print(save_name)
rec <- F
for(i in 1:ncol(degs)){ # for every cluster
  for(j in 1:length(x)){ # using x top genes
