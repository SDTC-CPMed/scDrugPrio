#
# CALCULATION OF MICROARRAY BASED DEGs
# FROM HUMAN SYNOVIUM AND PBMCs
#
# BY SAMUEL SCHAEFER
#################################################################
...
if (!requireNamespace("BiocManager", quietly = TRUE)) {
  stop(
    "Package \'BiocManager\' must be installed to run this function",
    call. = FALSE
  )
}
if (!requireNamespace("GEOquery", quietly = TRUE)) {
  stop(
    "Package \'GEOquery\' must be installed to run this function",
    call. = FALSE
  )
}
if (!requireNamespace("limma", quietly = TRUE)) {
  stop(
    "Package \'limma\' must be installed to run this function",
    call. = FALSE
  )
}
library(BiocManager)
...
degs <- GSE93272()
write.table(degs, file = "data/AIA/Microarray_DEGs/whole_blood_DEGs_GSE93272.txt", sep="\t", col.names = NA, row.names = T)
degs <- GSE55235()
write.table(degs, file = "data/AIA/Microarray_DEGs/synovium_DEGs_GSE55235.txt", sep="\t", col.names = NA, row.names = T)
