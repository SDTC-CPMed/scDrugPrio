...
#INPUT
###########################################################
ranking <- as.matrix(read_xlsx(path = "../Output/Final_ranking/FINAL_drug_ranking_with_previous_lit_evidence_evaluated.xlsx", sheet = 1))

ms_drug <- read.table(file = "../Input/Drugs/MS_drugs_from_DrugBank.txt",sep="\t", header = T, stringsAsFactors = F, quote = "")
n_ms_drug <- nrow(ms_drug)
...
for(i in 1:length(intervall)){
  temp[i] <- intervall[i]
  temp2[i] <- sum(known_drugs[as.numeric(ranking[,10]) <= intervall[i]])
  temp3[i] <- sum(as.numeric(ranking[,10]) <= intervall[i])
}
known_drugs <- cbind("n_rank" = temp, "n_drugs" = temp3, "n_known" = temp2, "precision" = (temp2*100)/temp3)

# LITERATURE EVIDENCE
lit_drugs <- grepl(ranking[as.numeric(ranking[,10]) <= 100,13], pattern = "Yes")
...
for(i in 1:length(intervall)){
  temp[i] <- intervall[i]
  temp2[i] <- sum(lit_drugs[as.numeric(ranking[,10]) <= intervall[i]])
  temp3[i] <- sum(as.numeric(ranking[,10]) <= intervall[i])
}
...
write.table(plot, file = paste(dir, "Precision_among_ranked_candidates.txt",sep=""),sep="\t", col.names = T, row.names = F)
...
p <- ggplot(data = plot, aes(x = n_drugs, y = precision, group = type)) + 
  ...
  ylab("Precision (%)") +
  scale_y_continuous(breaks = seq(0,max(plot$precision)+10, by = 10), limits = c(0,30)) +
...
ggsave(filename = paste(dir, "PRECISION_for_MS_drugs_among_ranked_candidates.pdf", sep=""), plot = p, device = "pdf", width = 4, height = 2.5, units = "in")
