...
MS_drugs <- as.matrix(read.table(file = "../Input/Drugs/MS_drugs_from_DrugBank.txt", sep="\t", stringsAsFactors = F, header = T, quote = ""))
...
out <- foreach(i = (1:length(lf)), .combine = "rbind") %do% {
  ...
  temp <- unlist(strsplit(strsplit(lf[i], split = "_")[[1]][4], split = ".txt"))[1] # get cluster number
...
clust_colors <- read.table(file = "../Output/MS/Cluster_colors.txt",sep="\t", stringsAsFactors = F, header = T)
fc_criterium_file <- read.table(file = "../Output/FC_criteria_checking/SUMMARY_only_drugs_passing_network_criteria_evaluated.txt",sep="\t",stringsAsFactors = F,header = T, quote = "")
...
z_plot$zc <- as.numeric(as.character(z_plot$zc))
z_plot$known_disease_specific_drug <- factor(x = z_plot$known_disease_specific_drug, levels = c("FALSE","TRUE"), ordered = T)

print(max(z_plot$zc))
print(min(z_plot$zc))

# Basic dot plot
p <- ggplot(z_plot, aes(x=Cluster, y=zc, fill=known_disease_specific_drug)) +
...
ggsave(filename = paste(dir, "z_scores_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 200, units = "mm")

p <- ggplot(z_plot, aes(x=Cluster, y=zc, fill=known_disease_specific_drug)) +
  geom_violin(aes(fill = known_disease_specific_drug), scale = 3) +
  #scale_fill_manual(breaks = c("TRUE", "FALSE"), values = c("black", "darkgrey"))+
  theme(plot.background = element_blank(), 
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"),
        ...
        panel.grid.minor = element_blank())+
  ylim(-12,12) + scale_y_continuous(breaks = seq(-12,12,2))
ggsave(filename = paste(dir, "z_scores_for_known_disease_specific_drugs_violin.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 200, units = "mm")
...
d_plot$dc <- as.numeric(as.character(d_plot$dc))
d_plot$known_disease_specific_drug <- factor(d_plot$known_disease_specific_drug, levels = c("FALSE","TRUE"), ordered = T)

# Basic dot plot
p <- ggplot(d_plot, aes(x=Cluster, y=dc, fill=known_disease_specific_drug)) +
...
ggsave(filename = paste(dir, "mean_closest_distance_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 200, units = "mm")
...
p <- ggplot(d_plot, aes(x=Cluster, y=dc, fill=known_disease_specific_drug)) +
  ...
  theme(plot.background = element_blank(), 
        ...
        panel.grid.minor = element_blank()) +
  ylim(0,5) + scale_y_continuous(breaks = seq(0,5,1))
ggsave(filename = paste(dir, "mean_closest_distance_for_known_disease_specific_drugs_violin.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 200, units = "mm")


# PLOT FOR PRECISION AT z(d(c)) cut-offs
################################################################################################
cl <- unique(out[,9]) # clusters
n_MS <- length(unique(out[grepl(out[,2],pattern = "TRUE"),1]))
prec_rec_matrix <- foreach(c = c(1:length(cl)), .combine = "rbind") %do% {
  temp <- out[out[,9]==cl[c],]
  cut_offs <- seq(ceiling(min(as.numeric(temp[,5]))*10)/10,0,0.1)
  prec_rec <- foreach(cut = c(1:length(cut_offs)), .combine = "rbind") %do% {
    temp2 <- temp[as.numeric(temp[,5]) <= cut_offs[cut],2]
...
ggsave(filename = paste(dir, "PRECISION_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 200, height = 80, units = "mm")
...
p <- ggplot(prec_rec_matrix, aes(x = cut_off, y = recall, color = Cluster)) + 
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.key.size = unit(0.7,"lines"),
        legend.title = element_text(face = "bold", size = 10, color = "black"))
ggsave(filename = paste(dir, "RECALL_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 200, height = 80, units = "mm")
...
p <- ggplot(prec_rec_matrix, aes(x = recall, y = precision, color = Cluster)) + 
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.key.size = unit(0.7,"lines"),
        legend.title = element_text(face = "bold", size = 10, color = "black"))
ggsave(filename = paste(dir, "RECALL_PRECISION_CURVES_for_known_disease_specific_drugs.pdf", sep=""), plot = p, device = "pdf", width = 200, height = 80, units = "mm")
...
z_0.15_criterium <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  ...
  temp <- temp[as.numeric(temp[,5]) < -0.15,2]
...
z_criterium <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  temp <- out[as.numeric(out[,9])==as.numeric(cl[i]),]
  temp <- temp[intersect(which(as.numeric(temp[,8]) < 0.05),which(as.numeric(temp[,5]) < 0)),2]
  prec <- sum(grepl(temp, pattern = "TRUE")) / length(temp)
  rec <- sum(grepl(temp, pattern = "TRUE")) / n_MS
...
z_and_d_criteria <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  temp <- out[as.numeric(out[,9])==as.numeric(cl[i]),]
  temp <- temp[intersect(which(as.numeric(temp[,8]) < 0.05),which(as.numeric(temp[,5]) < 0)),]
  temp <- temp[as.numeric(temp[,4])<1,2]
  if(length(temp)>0){
    prec <- sum(grepl(temp, pattern = "TRUE")) / length(temp)
    rec <- sum(grepl(temp, pattern = "TRUE")) / n_MS
  } else {
...
z_d_fc_criteria <- foreach(i = c(1:length(cl)), .combine = "rbind") %do% {
  if(any(fc_criterium_file[,19]==paste("FC_INDIVIDUAL_DRUGS_Cluster_", cl[i], "_res=0.35_dims=32_k=15",sep=""))){
    temp <- fc_criterium_file[fc_criterium_file[,19]==paste("FC_INDIVIDUAL_DRUGS_Cluster_", cl[i], "_res=0.35_dims=32_k=15",sep=""),]
...
ggsave(filename = paste(dir, "PRECISION_for_known_disease_specific_drugs_SELECTION_CRITERIA.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 175, units = "mm")
...
p <- ggplot(combined_rec_prec, aes(x = Cluster, y= Recall, fill = Cluster, color = Cluster)) +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.key.size = unit(1.5,"lines"),
        legend.title = element_text(face = "bold", size = 15, color = "black"))
ggsave(filename = paste(dir, "RECALL_for_known_disease_specific_drugs_SELECTION_CRITERIA.pdf", sep=""), plot = p, device = "pdf", width = 300, height = 175, units = "mm")
...
ggsave(filename = paste(dir, "CORRELATION_PRECISION_CENTRALITY_for_MS.pdf", sep=""), plot = p, device = "pdf", width = 200, height = 100, units = "mm")
