...
# transl
transl <- read.table(file = "../Input/Human-mouse_homologs/transl.txt", header = T, stringsAsFactors = F, sep="\t")
transl <- transl[,c(2,6)]
...
same_drugs <- same_drugs[same_drugs[,2] %in% ra_drugs,]
same_drugs <- same_drugs[same_drugs[,1] %in% colnames(drugs),]
ra_drug_info <- cbind(ra_drug_info, targets_in_lit_PPI = NA)
