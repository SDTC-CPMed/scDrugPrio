...
#'
#' @examples
#' Checks for the LCC formed by 200 unique nodes in the LCC
...
extract_LCC_by_gene_set_of_interest <- function(network, gene_set_of_interest, plot = F){
  ...
  if(!length(subGr)==0){
    ...
    if(length(subGr)==1){ # all elements in gene_set_of_interest form one LCC
      out <- matrix(out, ncol = 1)
    }
    if(is.vector(out)){
      if(length(subGr)>1){  # none of the elements in gene_set_of_interest form a connection with each other -> NO LCC
        out <- NULL
      }
    }
    # Decide which is the biggest connected component
