...
#' @param drug_target_matrix Matrix listing each drug's targets. One column per drug.
...
#'
#' @return Returns output matrix specifying network distances of the inputted drugs.
#' @export
#'
#' @examples
#' load_example_data()
...
average_closest_distance_network_drug_screening <- function(ppin,
  ...
  if(recycle){
    print(list.files(path= paste(out_dir, "temp",sep="")))
    # ppin_dist
    ppin_dist <- read.table(file = paste(out_dir, "temp/ppin_distances.txt",sep=""), sep="\t", header = F, stringsAsFactors = F)
    ...
    bins <- read.table(file = paste(out_dir, "temp/bins.txt", sep=""), sep="\t", header = F, stringsAsFactors = F)
    # all_genes
    all_genes <- as.vector(read.table(file = paste(out_dir, "temp/all_genes.txt", sep=""), sep="\t", header = F, stringsAsFactors = F)[,1])
    print("RECYCLING FILES")

  } else {

    dir.create(path = paste(out_dir, "temp", sep=""), showWarnings = F)
    ...
    if(prepare_recycle){  ################ SAVE ALL_GENES ################
      write.table(matrix(all_genes, ncol=1), file = paste(out_dir, "temp/all_genes.txt", sep=""), sep="\t", col.names = F, row.names = F)
    }

    # Creating / Loading bins
    bins <- bin_creation_by_min_bin_size(ppin = ppin, min_bin_size = min_bin_size)
    if(prepare_recycle){  ################ SAVE BINS ################
      write.table(bins, file = paste(out_dir, "temp/bins.txt", sep=""), sep="\t", col.names = F, row.names = F)
    }
    ...
    if(prepare_recycle){ ################ SAVE PPIN DISTANCES ################
      write.table(ppin_dist, file = paste(out_dir, "temp/ppin_distances.txt", sep=""), sep="\t", col.names = NA, row.names = T)
    }
  ...
  write.table(random_bin_drugs, file = paste(out_dir, "random_bin_drugs_",file_name,".txt", sep=""), sep = "\t", col.names = colnames(drug_target_matrix), row.names = F)
  ...
  out <- foreach(i = 1:ncol(drug_target_matrix), .combine = rbind, .packages = "scDrugPrio") %dopar% {
    temp <- rep(NA, times = 6)
    # n targets
    temp[1] <- n_targets[i]
  ...
  write.table(out, file = paste(out_dir, "drug-disease_closest_distances_vs_random_bin_adjusted__",file_name,".txt", sep=""), sep = "\t", col.names = T, row.names = F)
