...
#'
#' @return Returns a matrix including all cell type's ligand activities.
#' @export
#'
#'
NicheNet_ligand_activity_analysis <- function(degs,
    ...
    if(length(sender_degs)>0){
      out <- foreach(j = c(1:ncol(background_genes)), .combine = "rbind") %do% {
        ...
        if(exists("ligand_activity")){
          ...
          if(nrow(ligand_activity) > 1){
            ligand_activity <- ligand_activity[order(ligand_activity[,4], decreasing = T),]
          }
          if(ncol(ligand_activity) == 4){
            ligand_activity <- cbind(ligand_activity, NA)
          }
          ...
          ligand_activity <- matrix(NA, nrow = 1, ncol = 5)
        }
  ...
  ######################################################################
  for(i in 1:length(unique(all_ligand_activity[,6]))){
    if(i == 1 && exists("overview_ligand_activity")){
      rm(overview_ligand_activity)
    }
    for(j in 1:length(unique(all_ligand_activity[,7]))){
      t_i <- unique(all_ligand_activity[,6])[i]
      t_j <- unique(all_ligand_activity[,7])[j]

      temp <- all_ligand_activity

      if(sum(temp[,6]== t_i)>1){
        temp <- temp[temp[,6]== t_i,]
        if(sum(temp[,7]== t_j)>1){
          temp <- temp[temp[,7]== t_j,]
          temp <- nrow(temp)
        } else {
          if(sum(temp[,7]== t_j)==1){
            temp <- 1
        ...
        if(sum(temp[,6]== t_i)==1){
          if(which(temp[,6]==t_i) %in% which(temp[,7] == t_j)){
            temp <- 1
          } else {
            temp <- 0
          }
